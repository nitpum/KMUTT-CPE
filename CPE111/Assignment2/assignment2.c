...
int command (char name[], int paraCount, int minDigit, char cmd[], char p1[], char p2[], char p3[], int pCount, int * err) {
    ...
    if (strcmp(name,cmd) == 0) {
        if (paraCount <= pCount) {
            if (paraCount >= 1 && is_number(p1) == 0) *err = 1;
            if (paraCount >= 2 && is_number(p2) == 0) *err = 1;
            if (paraCount >= 3 && is_number(p3) == 0) *err = 1;
            if (*err == 1) return 1;

            if (minDigit > 0) {
                int digitCount = 0;
                if (pCount >= 1) digitCount += is_number(p1);
                if (pCount >= 2) digitCount += is_number(p2);
                if (pCount >= 3) digitCount += is_number(p3);
                if (digitCount < minDigit) { *err = 1; return 1; }
            } else if (paraCount < pCount) { *err = 1; return 1; }
...
int main () {
    ...
    do
    {
        ...
        printf("command> ");
        gets(input);
        count = 0; count = getparameter(input, cmd, p1, p2, p3);
        err = -1;
        toLower(cmd);
